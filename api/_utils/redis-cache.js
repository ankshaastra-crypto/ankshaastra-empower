...
/**
 * Redis Cache Manager
 * Handles caching for templates, static data, and frequently accessed data// Suppress DEP0169 deprecation warning from dependencies
...
class RedisCache {
  ...
  async initialize() {
    ...
    try {
      ...
      // Dynamic import — prevents ioredis from crashing module load in Vercel ESM
      let Redis;
      try {
        const mod = await import('ioredis');
        Redis = mod.default || mod;
      } catch (importErr) {
        console.warn('ioredis not available:', importErr.message);
        this.isConnected = false;
        this.client = null;
        return null;
      }
