...
export const DB_SCHEMA = 'public'; // D1 doesn't use schemas
...
export async function savePayment(orderId, transactionId, amountPaise, status) {
  const d1 = getD1Instance();
  if (d1) return d1SavePayment(d1, orderId, transactionId, amountPaise, status);

  const { savePayment: pgSave } = await import('./db.js');
  return pgSave(orderId, transactionId, amountPaise, status);
}
...
// ─── getPool ────────────────────────────────────────────────────────────────
// Returns null for D1 (no pool), or pg Pool if available
