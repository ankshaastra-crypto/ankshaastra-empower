...
export default async function handler(req, res) {
  ...
  if (!name?.trim() && !body?.fatherFullName?.trim()) {
    return res.status(400).json({ success: false, error: 'Name is required' });
  }
  // For baby packages, name comes from fatherFullName
  const resolvedName = name?.trim() || body?.fatherFullName?.trim() || 'Customer';
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email.trim())) {
    return res.status(400).json({ success: false, error: 'Invalid email' });
  }
  // Strip non-digits (handles +91, spaces, dashes)
  const cleanMobile = (mobile || '').replace(/\D/g, '');
  // Accept 10-digit Indian numbers, or 12-digit with 91 country code
  const normalizedMobile = cleanMobile.length === 12 && cleanMobile.startsWith('91')
    ? cleanMobile.slice(2)
    : cleanMobile.length === 13 && cleanMobile.startsWith('091')
    ? cleanMobile.slice(3)
    : cleanMobile;
  if (!/^[6-9]\d{9}$/.test(normalizedMobile)) {
    console.warn('Mobile validation failed for:', mobile, '-> cleaned:', cleanMobile);
    return res.status(400).json({ success: false, error: 'Invalid mobile number. Please enter a 10-digit Indian mobile number.' });
  }
