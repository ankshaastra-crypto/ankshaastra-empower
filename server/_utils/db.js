...
//         customerData.lastNameSpellingChangeOk || null, // $42
//         customerData.state             || null,   // $43
//       ]
//     );
...
// ─── isEmailSent ────────────────────────────────────────────────────────────────
// Informational check only — kept for any read-only status reporting.
// NOT safe for deduplication by itself; use tryReserveEmailSend() for that.
export async function isEmailSent(email, orderId) {
  const p = getPool();
  if (!p || !email || !orderId) return false;
  try {
...
// ─── tryReserveEmailSend ────────────────────────────────────────────────────────
// FIX (per client request 2026-08-02): replaces the racy check-then-send-
// then-record dedup pattern. The old flow ran a SELECT (isEmailSent) first,
// sent the email, and only afterwards ran an INSERT to mark it sent. If
// sendPaymentEmail() was called more than once for the same order within a
// short window — e.g. by payment-webhook.js AND payment-status.js, or a
// Razorpay webhook retry — every concurrent call could pass the SELECT
// before any of them finished the INSERT, so all of them sent the email.
// That's exactly why the admin inbox received the same order 3 times.
//
// This does the INSERT first (guarded by the existing UNIQUE(email,
// order_id) constraint) as an atomic reservation, BEFORE anything is sent.
// Only the one caller whose INSERT actually succeeds (returns a row) is
// allowed to send; every other concurrent caller sees the unique-constraint
// conflict and skips sending. There's no window where two callers can both
// believe they're first.
export async function tryReserveEmailSend(email, orderId) {
  const p = getPool();
  if (!p || !email || !orderId) return true; // no DB — fail open, allow send
  await ensureSchemaOnce();
  try {
    const result = await p.query(
      `INSERT INTO ${DB_SCHEMA}."emailDelivery" (email, order_id, status, sent_at)
    ...
    return result.rowCount > 0; // true = we won the reservation, safe to send
  } catch (error) {
    console.error('tryReserveEmailSend failed (allowing send to avoid blocking):', error.message);
    return true; // Fail open
  }
}

// ─── markEmailSent ──────────────────────────────────────────────────────────────
// Call after a reserved email has actually been sent successfully.
export async function markEmailSent(email, orderId) {
  const p = getPool();
  if (!p || !email || !orderId) return false;
  try {
    await p.query(
      `UPDATE ${DB_SCHEMA}."emailDelivery" SET status = 'sent', sent_at = NOW()
...
// If sending fails after a successful reservation, remove the pending row so
// a future retry for this order isn't permanently blocked.
export async function releaseEmailReservation(email, orderId) {
  const p = getPool();
  if (!p || !email || !orderId) return false;
  try {
    await p.query(
      `DELETE FROM ${DB_SCHEMA}."emailDelivery"
       WHERE email = $1 AND order_id = $2 AND status = 'pending'`,
      [email, orderId]
    );
    return true;
  } catch (error) {
    console.error('releaseEmailReservation error:', error.message);
    return false;
  }
}

// ─── recordEmailDelivery ───────────────────────────────────────────────────────
// Kept for any other call sites still using the old pattern directly.
export async function recordEmailDelivery(email, orderId, deliveryStatus = 'sent') {
  const p = getPool();
  if (!p || !email || !orderId) return false;
  await ensureSchemaOnce();
  try {
    await p.query(
      `INSERT INTO ${DB_SCHEMA}."emailDelivery" (email, order_id, status, sent_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (email, order_id) DO UPDATE SET status = EXCLUDED.status, sent_at = NOW()`,
      [email, orderId, deliveryStatus]
    );
    return true;
  } catch (error) {
