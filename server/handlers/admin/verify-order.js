    ...
    if (orderId) {
      const result = await pool.query(`
        SELECT
          ...
          c.child_middle_name, c.child_last_name, c.father_first_as_middle,
          c.name_options, c.last_name_spelling_ok,
    ...
    fields: {
      ...
      ...(isBaby ? {
        babyReport: {
          ...
          lastNameSpellingOk: row.last_name_spelling_ok ? '✅' : '⚠️ empty',
        },
      } : {}),
      ...(isNameCheck ? {
        nameCheck: {
...
function getIssues(row) {
  ...
  if (isBaby) {
    ...
    if (!row.last_name_spelling_ok) issues.push('Missing last name spelling preference');
  }
