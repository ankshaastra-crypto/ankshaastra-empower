...
//     const lastNameSpellingChangeOk    = get('lastNameSpellingChangeOk');
...
//           fatherFirstNameAsMiddleName,        lastNameSpellingChangeOk, nameOptions,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // STEP 7: Send WhatsApp on SUCCESS only.
    //
    // Empower's own PDF-generation + email-sending (sendPaymentEmail) has
    // been removed from this path — it was firing independently of
    // server/handlers/payment-webhook.js, so customers were getting 2
    // confirmation emails per order (this old-format one, plus the new hub
    // CRM email triggered from payment-webhook.js's syncOrderToHubCrm()).
    // The hub now owns the customer-facing confirmation email. This stub
    // keeps the response JSON's `email.*` fields unchanged for any caller
    // that reads them.
    // ─────────────────────────────────────────────────────────────────────────
    const emailResult = {
      success: false,
      skipped: true,
      reason: 'disabled: hub CRM email (sent via payment-webhook.js) replaces this',
    };
