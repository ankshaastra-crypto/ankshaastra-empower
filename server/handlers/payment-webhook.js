...
//                 lastNameSpellingChangeOk:      r.last_name_spelling_ok,
...
//     const finalLastNameSpellingChangeOk   = str(metadata.lastNameSpellingChangeOk);
...
//     // Sync customer + order into the shared Ankshaastra hub CRM (SUCCESS
//     // only). Guarded by the same atomic reservation used for email dedup —
//     // Razorpay retries this webhook (often 3-4x within seconds), and
//     // without a guard each retry made the hub's own automation re-fire a
//     // new invoice number + email. The reservation key is prefixed
//     // "hubsync:" so it never collides with the real email-send reservation
//     // for the same order. Best-effort and non-blocking — never throws,
//     // never affects Empower's own DB/email/invoice/WhatsApp flow below.
//     if (status === 'SUCCESS') {
//       try {
//         const { tryReserveEmailSend } = await import('../_utils/db.js');
//         const hubSyncReserved = await tryReserveEmailSend(`hubsync:${finalCustomerEmail || 'noemail'}`, orderId);
//         if (hubSyncReserved) {
//           const { syncOrderToHubCrm } = await import('../_utils/sync-hub-customer.js');
//           await syncOrderToHubCrm({
//             name: finalCustomerName,
//             email: finalCustomerEmail,
//             mobile: finalCustomerMobile,
//             packageTitle: finalPackageType,
...
//           });
//         } else {
//           console.log(`⏭️  Hub CRM sync already done for order ${orderId} — skipping duplicate sync`);
//         }
//       } catch (hubSyncError) {
//         console.error('[payment-webhook] Hub CRM sync failed (non-fatal):', hubSyncError?.message || hubSyncError);
//       }
//     }
...
//     // Sync customer + order into the shared Ankshaastra hub CRM (SUCCESS
//     // only). Best-effort and non-blocking — never throws, never affects
//     // Empower's own DB/email/invoice/WhatsApp flow below.
//     if (status === 'SUCCESS') {
//       try {
//         const { syncOrderToHubCrm } = await import('../_utils/sync-hub-customer.js');
//         await syncOrderToHubCrm({
          ...
          if (p) {
            ...
            if (result.rows.length > 0) {
              ...
              metadata = {
                ...
                pinCode: r.pin_code,
                state: r.state,
                parentsProfession: r.parents_profession,
                packageType: r.package_type,
                person1Name:           r.person1_name,
                person1FirstName:      r.person1_first_name,
                person1MiddleName:     r.person1_middle_name,
