...
import { CLIENTS, fmtDate, type ReportStatus } from "../data/seed";
...
export default function Reports() {
  ...
  const [items, setItems] = useState(CLIENTS.map(c => ({ id: c.id, name: c.name, service: c.service, status: c.reportStatus, date: c.dateAdded })));
  ...
  const move = (id: string, dir: 1 | -1) => {
    ...
    toast("Moved to next stage");
  };

  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-semibold gold-gradient-text">Reports Pipeline</h1>
      <p className="text-sm text-[hsl(var(--muted-foreground))]">Click arrows on cards to move between stages.</p>
                  ...
                  {cards.length === 0 ? (
                    ...
                    return (
                      <div key={c.id} className="rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--navy-2))] p-3 hover:border-[hsl(var(--gold)/0.4)] transition-colors">
                        <Link to={`/admin/v2/clients/${c.id}`} className="font-medium text-sm hover:text-[hsl(var(--gold))]">{c.name}</Link>
