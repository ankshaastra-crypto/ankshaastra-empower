    ...
    <section className="py-12 md:py-20 lg:py-28 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #2E1A47 0%, #0F0E1A 100%)' }} ref={ref}>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto">
          {/* Root Cause Card */}
          <div className={`relative bg-white/5 backdrop-blur-xl rounded-2xl md:rounded-3xl p-6 md:p-12 border border-accent/30 gold-glow transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`}>
            {/* Golden Corner Accents */}
            <div className={`absolute top-0 left-0 w-12 md:w-20 h-12 md:h-20 border-t-2 border-l-2 border-accent/50 rounded-tl-2xl md:rounded-tl-3xl transition-all duration-500 delay-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`} />
            ...
            <div className="text-center">
              <h2 className={`text-xl md:text-3xl font-heading font-bold text-accent mb-4 md:mb-6 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                The Root Cause:
              </h2>
