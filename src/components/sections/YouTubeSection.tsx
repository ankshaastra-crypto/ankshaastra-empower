...
const YouTubeSection = () => {
  ...
  return (
    <section
      ref={ref}
      className={`py-12 md:py-16 lg:py-20 relative overflow-hidden section-hidden ${isVisible ? 'section-visible' : ''}`}
      ...
      <div className="container mx-auto px-5 sm:px-6 relative z-10">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 md:mb-10">
            ...
            <p className="text-sm md:text-lg text-white/70 max-w-2xl mx-auto px-2 leading-relaxed">
              Learn how numerology and Vedic principles can shape your child's name and destiny.
            </p>
          </div>

          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 stagger-children ${isVisible ? 'section-visible' : ''}`}>
            {videos.map((video, index) => (
              <div
                key={index}
                className="relative rounded-xl md:rounded-2xl p-4 md:p-4 border border-white/10 group hover:border-white/20 transition-all duration-300 hover:-translate-y-2 bg-white/5"
              >
