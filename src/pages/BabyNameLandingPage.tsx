...
function validate(d: FormData): FormErrors {
  ...
  if (!d.placeOfBirth || d.placeOfBirth.trim().length < 2) e.placeOfBirth = "Place of birth is required.";
  if (!d.state || d.state.trim().length < 2) e.state = "State / Province is required.";
...
const ClaimFormSection = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setTouched({ yourName: true, fatherFirstName: true, fatherLastName: true, dob: true, timeOfBirth: true, placeOfBirth: true, state: true, gender: true, email: true, whatsapp: true });
      ...
      <div ref={ref} style={{ ...S.sectionInner("640px") }}>
        ...
        <div className="bn-form-pad" style={{
          ...
          <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: 22 }}>
            ...
            {/* 6. City of Birth */}
            <div>
              <label style={S.label()}><Baby size={15} color={T.gold} /> City of Birth *</label>
              <Input value={formData.placeOfBirth} onChange={e => handleChange("placeOfBirth", e.target.value)}
                onBlur={() => handleBlur("placeOfBirth")} placeholder="e.g. Mumbai"
                maxLength={80}
            ...
            {/* 7. State / Province (mandatory for GST invoice) */}
            <div>
              <label style={S.label()}><Baby size={15} color={T.gold} /> State / Province *</label>
              <Input value={formData.state} onChange={e => handleChange("state", e.target.value)}
                onBlur={() => handleBlur("state")} placeholder="Type to search e.g. Maha…"
                list={STATE_DATALIST_ID} autoComplete="off"
                maxLength={60}
                style={{ ...inputSx, border: fieldBorder("state"), height: 48 }}
                className="focus-visible:ring-0" />
              <datalist id={STATE_DATALIST_ID}>
                {ALL_STATE_OPTIONS.map((s) => (<option key={s} value={s} />))}
              </datalist>
              {touched.state && errors.state && <p style={{ color: "#DC2626", fontSize: "0.78rem", marginTop: 4, fontFamily: T.body }}>{errors.state}</p>}
              <p style={{ color: T.secondary, fontSize: "0.72rem", marginTop: 4, fontFamily: T.body }}>Required for GST invoice compliance.</p>
            </div>
